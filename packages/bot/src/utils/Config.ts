...
const env = config().parsed ?? config({ path: path.resolve('../../.env') }).parsed;
...
export const CONFIG: ConfigType = {
  ...
  dbPath: env.bot_db_path ?? path.join(process.cwd(), 'db.sqlite'),
  logPath: env.bot_db_path ?? path.join(process.cwd(), 'log.sqlite'),
};
