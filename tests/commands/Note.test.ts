...
import { Command, IMessageCommand, INoteCommand } from '../../types/models/Command';

let db: ReturnType<typeof createMockDb>;
let bot: ReturnType<typeof createMockBot>;

// helper to create a NoteCommand instance
const createTestNoteCommand = (overrides?: Partial<Command<INoteCommand>>): Command<INoteCommand> => {
  return {
    ...
    ...overrides,
    opts: {
      ...NoteCommand.defaultConfig.opts,
      ...(overrides?.opts ?? {}),
    },
  };
};
...
test('returns false if missing command name', () => {
  ...
  const cmd = createTestNoteCommand();
